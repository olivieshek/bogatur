...
Вы настолько были удивлены, что вырвался крик:''')
            ...
            dice_han += dice_han1
            print("Ему выпало %s. Всего это - %s." % (dice_han1, dice_han))

        if dice_han <= dice:
            print("- У тебя - %s, у меня - %s... Эх! Ладно, беги!" % (dice, dice_han1))
            level = 2
        else:
            print("- Так, дружок. У меня - %s, у тебя - %s. Понимаешь, что это значит?" % (dice_han, dice))
            print("Два разбойника, больше похожих на мамонтов, с обеих сторон схватили Вас...")
            sys.exit("Ну Вы же знаете, что дальше было!")

    print("Вы бежали обратно со всех ног. Но Вы остались в живых, хотя это, наверное, - чистое везение.")

    print('''И вот Вы снова возле камня. Осталось две дороги: 
        2 - богатым быть,
        3 - женатым быть.''')
    user_choice = "2"
